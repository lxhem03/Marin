...
async def get_cache(db, key: str):
...
async def set_cache(db, key: str, value: Any, ttl: int = CACHE_TTL_SECONDS):
    doc = {"key": key, "value": json.dumps(value, default=json_util.default), "expires_at": int(time.time()) + int(ttl)}
...
async def search_and_cache(db, query: str, max_results: int = 100):
    ...
    await set_cache(db, key, normalized)
    return normalized
